...
function ShareReportSection({ onExport, isExporting, hasData, reportTitle, isProUser }: {
  onExport: (f: 'pdf' | 'pptx') => void; isExporting: string | null; hasData: boolean; reportTitle: string; isProUser: boolean;
}) {
  const [emailTo, setEmailTo] = useState('');
  const [emailMsg, setEmailMsg] = useState('');

  const shareWhatsApp = () => {
    window.open(`https://wa.me/?text=${encodeURIComponent(`Check out my data report: "${reportTitle}" — generated by DataVora`)}`, '_blank');
  };
  const shareGmail = () => {
    window.open(`mailto:?subject=${encodeURIComponent(`Data Report: ${reportTitle}`)}&body=${encodeURIComponent(`Hi,\n\nPlease find attached my data report "${reportTitle}" generated by DataVora.\n\nBest regards`)}`, '_blank');
  };
    ...
    <Card className="bg-card border-border">
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          <Share2 className="h-4 w-4" />Share Your Report
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-xl border border-border space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: 'hsl(217 91% 60% / 0.15)' }}>
                <Download className="h-4 w-4 text-primary" />
              </div>
              <div>
                <h4 className="text-sm font-semibold">Download & Share</h4>
                <p className="text-[10px] text-muted-foreground">Download and share via WhatsApp, Email, or Drive</p>
              </div>
            </div>
            <div className="flex gap-2">
              <Button size="sm" className="flex-1 h-8 text-xs" onClick={() => onExport('pdf')} disabled={!hasData || !!isExporting}>
                <Download className="h-3 w-3 mr-1" />PDF
              </Button>
              <Button size="sm" className="flex-1 h-8 text-xs" onClick={() => onExport('pptx')} disabled={!hasData || !!isExporting}>
                <Download className="h-3 w-3 mr-1" />PPTX
              </Button>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="flex-1 h-7 text-[10px]" onClick={shareWhatsApp}>📱 WhatsApp</Button>
              <Button variant="outline" size="sm" className="flex-1 h-7 text-[10px]" onClick={shareGmail}>📧 Gmail</Button>
            </div>
          </div>

          {/* Option 2: Share Link (Pro) */}
          <div className="p-4 rounded-xl border border-border space-y-3 relative">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: 'hsl(271 91% 65% / 0.15)' }}>
                <Link className="h-4 w-4" style={{ color: '#7C3AED' }} />
              </div>
              <div>
                <h4 className="text-sm font-semibold">Share via Link</h4>
                <p className="text-[10px] text-muted-foreground">Generate a shareable link anyone can view</p>
              </div>
            </div>
            {!isProUser ? (
              <div className="text-center py-3 space-y-2">
                ...
                <Button variant="outline" size="sm" className="text-[10px] h-7">Upgrade to share reports online →</Button>
              </div>
            ) : (
              <div className="space-y-2">
                <Input className="h-8 text-xs" readOnly value="https://datavora.app/report/abc123" />
                <Button size="sm" className="w-full h-8 text-xs">Copy Link</Button>
                <div className="flex gap-2 text-[10px] text-muted-foreground">
                  <span>Expiry: 7 days</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-4 rounded-xl border border-border space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: 'hsl(160 84% 39% / 0.15)' }}>
                <Mail className="h-4 w-4" style={{ color: '#059669' }} />
              </div>
              <div>
                <h4 className="text-sm font-semibold">Email Report</h4>
                <p className="text-[10px] text-muted-foreground">Send report directly to any email</p>
              </div>
            </div>
            <Input className="h-8 text-xs" placeholder="recipient@email.com" value={emailTo} onChange={e => setEmailTo(e.target.value)} />
            <Textarea className="text-xs min-h-[60px] resize-none" placeholder="Optional message..." value={emailMsg} onChange={e => setEmailMsg(e.target.value)} />
            <Button size="sm" className="w-full h-8 text-xs" disabled={!emailTo || !hasData}
              onClick={() => { toast({ title: 'Email sent!', description: `Report sent to ${emailTo}` }); setEmailTo(''); setEmailMsg(''); }}>
...
function ScheduledReportsProCard({ isProUser }: { isProUser: boolean }) {
  if (isProUser) return null;
  return (
    <Card className="bg-card border-border" style={{ borderColor: '#4C1D95' }}>
      <CardContent className="py-6">
        <div className="flex items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ backgroundColor: 'hsl(271 91% 65% / 0.15)' }}>
              <Clock className="h-6 w-6" style={{ color: '#7C3AED' }} />
            </div>
            <div>
              <h3 className="font-bold text-base">Scheduled Reports</h3>
              <p className="text-sm text-muted-foreground">Automate weekly/monthly report delivery to your team's inbox</p>
              <ul className="mt-2 text-xs text-muted-foreground space-y-0.5">
                <li>• Set frequency: Daily / Weekly / Monthly</li>
                <li>• Choose format: PDF or PPTX</li>
                <li>• Add unlimited recipients</li>
                <li>• Auto-generates from latest data</li>
              </ul>
            </div>
          </div>
          <div className="text-center shrink-0">
            <Lock className="h-6 w-6 mx-auto text-muted-foreground mb-1" />
            <Badge className="text-[10px] mb-2" style={{ backgroundColor: '#D97706', color: '#fff' }}>Pro Feature</Badge>
            <p className="text-sm font-bold">₹999/month</p>
            <Button size="sm" className="mt-2 text-xs">Upgrade to Unlock →</Button>
          </div>
        </div>
      </CardContent>
    </Card>
...
function ActivityLog({ history, onDelete, formatBadge }: {
  history: ReportHistoryItem[]; onDelete: (id: string) => void;
  formatBadge: Record<string, { color: string; label: string }>;
}) {
  return (
    <Card className="bg-card border-border">
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          <Clock className="h-4 w-4" />Report Activity
        </CardTitle>
      </CardHeader>
      <CardContent>
        {history.length === 0 ? (
          ...
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
            {history.map(h => (
              <div key={h.id} className="flex items-start gap-3 p-3 rounded-lg bg-muted/30">
                <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: `${formatBadge[h.format]?.color}20` }}>
                  <FileText className="h-4 w-4" style={{ color: formatBadge[h.format]?.color }} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <Badge className="text-[8px] text-white px-1 py-0" style={{ backgroundColor: formatBadge[h.format]?.color }}>
                      {formatBadge[h.format]?.label}
                    </Badge>
                    <span className="text-xs font-semibold truncate">{h.templateName} generated</span>
                  </div>
                  <p className="text-[10px] text-muted-foreground mt-0.5">
                    {h.sectionCount} sections • {new Date(h.generatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    {' • '}
                    {new Date(h.generatedAt).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                  </p>
                </div>
                <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive shrink-0" onClick={() => onDelete(h.id)}>
...
function ReportNotes() {
  ...
  const saveNote = () => {
    ...
    const updated = [{ text: draft.trim(), date: new Date().toISOString() }, ...notes].slice(0, 20);
    setNotes(updated);
  ...
  return (
    <Card className="bg-card border-border">
      ...
      <CardContent className="space-y-3">
        <Textarea className="text-sm min-h-[80px] resize-none" placeholder="Add notes about this report..."
          value={draft} onChange={e => setDraft(e.target.value)} />
        <Button size="sm" className="w-full h-8 text-xs" onClick={saveNote} disabled={!draft.trim()}>
          Save Note
        </Button>
        {notes.length > 0 && (
          <div className="space-y-2 max-h-[200px] overflow-y-auto pt-2">
            {notes.map((n, i) => (
              <div key={i} className="p-2.5 rounded-lg bg-muted/30 group">
