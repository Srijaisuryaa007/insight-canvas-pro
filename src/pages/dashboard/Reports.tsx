          ...
          <div className="p-4 rounded-xl border border-border space-y-3">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="flex-1 h-8 text-xs"
                onClick={() => window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out my data report: "${reportTitle}" — generated by DataVora`)}`, '_blank')}>
                📱 WhatsApp
              </Button>
              <Button variant="outline" size="sm" className="flex-1 h-8 text-xs"
                onClick={() => window.open(`https://mail.google.com/mail/?view=cm&su=${encodeURIComponent(`Data Report: ${reportTitle}`)}&body=${encodeURIComponent(`Hi,\n\nPlease find attached my data report "${reportTitle}" generated by DataVora.\n\nBest regards`)}`, '_blank')}>
                📧 Gmail
              </Button>
            </div>
          </div>
